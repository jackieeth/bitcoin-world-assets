...
export const genBitFeedMml = async (
  ...
  const modelWillAppear = deterministicRandom(`${day}-${hour}`) > 0.1; // Randomly decide if a model will appear
  ...
  for (let i = 0; i < txList.length; i++) {
    ...
    if (slot) {
      ...
      parcelsMML += `<m-cube id="parcel-${i}-size-${slot.r}" width="${
        ...
        slot.r * size * 0.9
      }" x="${(slot.position.x + slot.r - blockWidth / 2) * size - margin * slot.r}" y="${(0.1 * slot.r) / 2 - platform_thickness * slot.r/2 - 0.5}" z="${
        ...
        deterministicRandom(`b${i}${minute}`) > 0.995
          ...
          : ``
      } ${slot.r === 6 && placeModel && modelWillAppear ? `<m-model src="https://quark20a.s3.us-west-1.amazonaws.com/q/1377952728323592342_1748599938.glb" x="0" y="2" z="0" sx="2" sy="2" sz="2"> </m-model>`: ``}</m-cube>`;
      if (slot.r === 6) {
